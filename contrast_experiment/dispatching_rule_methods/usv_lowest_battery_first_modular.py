...
算法描述：
    1. 按照任务出现的顺序进行处理
    2. 为每个任务选择当前电量最低的USV（优先使用低电量USV）
    3. 考虑USV电量约束，必要时进行充电
"""
...
class LowestBatteryFirstPlanner(BasePlanner):
    """最低电量优先规划器"""
    ...
    def _select_usv_for_task(self, task: Task, usvs: List[USV], config: Dict) -> tuple:
        ...
        for usv in usvs:
            if self._can_execute_task(usv, task, config):
                capable_usvs_now.append((usv, usv.battery_level))
            else:
                # 检查充电后是否能执行
        ...
        # 优先选择当前就能执行的USV中电量最低的
        if capable_usvs_now:
            chosen_usv, _ = min(capable_usvs_now, key=lambda x: x[1])  # 按电量排序（升序）
...
def run_single_case(json_file: str, config: Dict = None) -> Dict:
    ...
    Returns:
        调度结果
    """
    planner = LowestBatteryFirstPlanner(config)
