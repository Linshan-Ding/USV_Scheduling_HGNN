...
class NearestTaskFirstPlanner(BasePlanner):
    """最近任务优先规划器"""
    ...
    def _execute_scheduling(self, tasks: List[Task], usvs: List[USV], config: Dict):
        ...
        Args:
            ...
            config: 环境配置
        """
        # 按距离基地从近到远排序任务（与最远任务优先相反）
        tasks.sort(key=lambda t: self._distance_from_base(t.position))
...
def run_single_case(json_file: str, config: Dict = None) -> Dict:
    ...
    Returns:
        调度结果
    """
    planner = NearestTaskFirstPlanner(config)
