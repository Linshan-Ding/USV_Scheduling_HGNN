...
class HighestBatteryFirstPlanner(BasePlanner):
    """最高电量优先规划器"""
    ...
    def _select_usv_for_task(self, task: Task, usvs: List[USV], config: Dict) -> tuple:
        ...
        # 优先选择当前就能执行的USV中电量最高的
        if capable_usvs_now:
            chosen_usv, _ = max(capable_usvs_now, key=lambda x: x[1])  # 按电量排序
...
def run_single_case(json_file: str, config: Dict = None) -> Dict:
    ...
    Returns:
        调度结果
    """
    planner = HighestBatteryFirstPlanner(config)
